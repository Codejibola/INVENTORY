...
useEffect(() => {
  ...
  // If month is a number (0-11), filter it
  if (typeof month === 'number') {
    filtered = dailySales.filter((item) => {
      // Split the string 'YYYY-MM-DD' to get the month directly
      // This avoids timezone bugs that hide February
      const monthPart = parseInt(item.date.split("-")[1], 10); 
      return monthPart === (month + 1);
    });
  }
...
const handleDownload = async (date) => {
  ...
  doc.text("Authorized Signature", pageWidth - 35, currentY + 35, { align: "center" });
    ...
    doc.save(`Report-${formatDate(date)}.pdf`);
  ...
  <div>
    ...
    <p className="text-gray-500 text-sm">See how much you made and save your records. Everything resets at midnight.</p>
  </div>

  {/* SIMPLE INFO BOX */}
  <div className="bg-blue-500/5 border border-blue-500/10 p-5 rounded-2xl">
    <p className="text-[10px] uppercase font-black text-blue-500 tracking-widest mb-3">How to use these reports</p>
    
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="space-y-1">
        <p className="text-sm font-bold text-white">Check your cash</p>
        <p className="text-xs text-gray-400 leading-relaxed">
          Compare the "Total Revenue" here with the money in your drawer to make sure everything matches perfectly.
        </p>
      </div>

      <div className="space-y-1">
        <p className="text-sm font-bold text-white">Keep proof of sales</p>
        <p className="text-xs text-gray-400 leading-relaxed">
          Download the PDF reports so you have a permanent copy of your sales for your own records or for tax time.
        </p>
      </div>

      <div className="space-y-1">
        <p className="text-sm font-bold text-white">See what sells</p>
        <p className="text-xs text-gray-400 leading-relaxed">
          Look at the "Profit" column to see which items are making you the most money so you know what to restock.
        </p>
      </div>
    </div>
  </div>
                  ...
                  <AnimatePresence mode='popLayout'>
                    {currentData.length === 0 ? (
                      <motion.tr 
                        ...
                        key="empty"
                      >
                        <td colSpan="3" className="px-6 py-20 text-center text-gray-600 italic">No Report records found for this period.</td>
                      </motion.tr>
                    ) : (
                      currentData.map((row) => (
                        <motion.tr 
